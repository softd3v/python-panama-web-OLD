<footer id="footer" class="border-0" style="
          background: url('img/parallax/Img_1920x1000xp_Nv.jpg');
          background-size: cover;
          background-position: 0 100%;
        ">
      <div class="container py-4">
        <div class="row justify-content-md-center py-5 mt-3">
          <div
            class="col-md-12 col-lg-3 d-flex align-items-center justify-content-center justify-content-lg-start mb-5 mb-lg-0">
            <a href="index.php"><img src="img/logo-dark.png" alt="Logo" class="img-fluid"
                style="max-width: 150px" /></a>
          </div>
          <div class="col-md-6 col-lg-2 text-center text-lg-left mb-5 mb-lg-0">
            <h5 class="text-5 text-transform-none font-weight-semibold text-color-light mb-4">
              Paginas
            </h5>
            <ul class="list list-unstyled">
              <!-- <li class="mb-1">
                <a href="page-services.html" class="link-hover-style-1">Nosotros</a>
              </li> -->
              <!-- <li class="mb-1">
                <a href="page-services.html" class="link-hover-style-1">Noticias</a>
              </li> -->
              <!-- <li class="mb-1">
                <a href="page-services.html" class="link-hover-style-1">Blog</a>
              </li> -->
              <li class="mb-1">
                <a href="contacto.php" class="link-hover-style-1">Contacto</a>
              </li>
            </ul>
          </div>
          <div class="col-md-6 col-lg-2 text-center text-lg-left mb-5 mb-lg-0">
            <h5 class="text-5 text-transform-none font-weight-semibold text-color-light mb-4">
              Enlaces
            </h5>
            <ul class="list list-unstyled">
              <li class="mb-1">
                <a href="https://www.python.org/" class="link-hover-style-1">
                  Python Sitio Oficial</a>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <div class="footer-copyright footer-copyright-style-2 background-transparent footer-top-light-border">
        <div class="container py-2">
          <div class="row py-4">
            <div class="col d-flex align-items-center justify-content-center mb-4 mb-lg-0">
              <p>© Copyright <?php echo date("Y"); ?>. Todos los derechos reservados.</p>
            </div>
          </div>
        </div>
      </div>
    </footer>